...
st.set_page_config(
    page_title="Factory Floor Optimizer",
    page_icon="🏭",
    layout="wide",
)
...
if len(st.session_state.path_points) > 0:
    try:
        ...
        if "Movement Mode" in st.session_state.path_points.columns:
            p_mode_list = (
                st.session_state.path_points["Movement Mode"].astype(str).tolist()
            )
...
if viewport_mode == "📐 2D ASME Y14.1 Blueprint View":
    ...
    fig = draw_asme_drawing(
        ...
        show_locator_dims=st.session_state.get("show_locator_dims", False),
        designer_name=st.session_state.designer_name,
        dwg_title=st.session_state.dwg_title,
        dwg_num=st.session_state.dwg_num,
    )
...
with stat4:
    st.metric(
        "UDP Power Sleep Savings",
        metrics.get("UDP Switch-Off Savings", "N/A"),
    )
...
if warnings:
    ...
    st.success("✅ No implemented spatial/compliance warnings detected.")
...
with tab_reports:
    ...
    report_bundle = build_full_report_bundle(
        st.session_state, active_workflow_paths
    )
    ...
    st.download_button(
        ...
        data=report_json,
        file_name=(
            f"factory_report_{st.session_state.dwg_num.replace(' ', '_')}.json"
        ),
