...
def draw_asme_drawing(
  ...
  if show_cranes:
    for idx, crane in enumerate(cranes):
      ...
      if show_locator_dims and bool(crane.get("dim_visible", True)):
        cx_ft = (float(crane.get("ll_x", 0.0)) + float(crane.get("ur_x", 0.0))) / 2.0
        cy_ft = (float(crane.get("ll_y", 0.0)) + float(crane.get("ur_y", 0.0))) / 2.0
        cx_in = O_x + cx_ft * S
        cy_in = O_y + cy_ft * S

        label_x_offset_in = float(crane.get("dim_label_x_offset_ft", 0.0)) * S
        label_y_offset_in = float(crane.get("dim_label_y_offset_ft", 0.0)) * S
        show_metadata = bool(crane.get("dim_show_metadata", True))

        note_lines = [str(crane.get("id", f"CR-{idx+1:03d}"))]

        if show_metadata:
          note_lines.append(
              f"{float(crane.get('ur_x', 0.0)) - float(crane.get('ll_x', 0.0)):.1f} x "
              f"{float(crane.get('ur_y', 0.0)) - float(crane.get('ll_y', 0.0)):.1f} ft"
          )
    ...
    for idx, l in enumerate(lighting):
      ...
      if show_locator_dims and bool(l.get("dim_visible", True)):
        ...
        _draw_dim_line(
            ...
            color=dim_color, lw=0.8, z=7
        )
        _draw_ext_line(
            ax, O_x, O_y, O_x, x_dim_y + 0.05,
        ...
        _draw_dim_line(
            ax, y_dim_x, O_y, y_dim_x, ly_in,
            color=dim_color, lw=0.8, z=7
        )
        _draw_ext_line(
            ax, O_x, O_y, y_dim_x + 0.05, O_y,
            color=ext_color, lw=0.7, z=6
        )
        _draw_ext_line(
        ...
        if dim_show_fixture_note:
          fixture_note = (
              f"{l.get('Type', 'Light')} / "
              f"{float(l.get('Wattage', 0.0)):.0f}W"
          )
          ax.text(
              lx_in + 0.12,
              ly_in - 0.12,
