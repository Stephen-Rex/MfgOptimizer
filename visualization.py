...
def draw_asme_drawing(size_char="B", scale_factor=10.0, machines=[], conduits=[], lighting=[], show_safety=False, show_contour=False):
    """
    Renders factory layout inside standardized ASME Y14.1 margins, title block,
    including machines, conduit lines, and lighting fixtures.
    """
    ...
    # Draw ASME border & format margin
    ...
    # Draw ASME Title Block (lower right)
    ...
    for l in lighting:
        lx, ly = l["x"], l["y"]
        # Plot yellow circle with a white border and lightbulb symbol star
        ax.plot(lx, ly, marker='o', color='gold', markersize=12, markeredgecolor='black', markeredgewidth=1)
        ax.plot(lx, ly, marker='*', color='white', markersize=6)
        ax.text(lx + 2, ly + 2, f"{l['Make']}\n{l['Brand']}", fontsize=5, color='darkgoldenrod', weight='bold')
